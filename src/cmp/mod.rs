...
pub enum Status {
    Equal(std::path::PathBuf, std::path::PathBuf),
    Partial(std::path::PathBuf, std::path::PathBuf),
    Missing(std::path::PathBuf),
}

pub fn files(left: std::path::PathBuf, right: std::path::PathBuf) -> Result<Status> {}
...
pub fn files(
    ...
    match (left.exists(), right.exists()) {
        ...
        (true, false) => Ok(Status::Missing(right)),
        (false, true) => Ok(Status::Missing(left)),
        (true, true) => {
            ...
            loop {
                ...
                if left_buffer[..left_read] != right_buffer[..right_read] {
                    break Ok(Status::Partial(left, right));
                }

                if left_read == 0 {
                    break Ok(Status::Equal(left, right));
                }
            }
        }
    }
...
fn dir_files(
    ...
    left: path::Paths,
    right: path::Paths,
) -> Result<Vec<path::PathStatus>> {
    if left.names.is_empty() {
        return make_missing(shared, left.parent, right.names);
    }

    if right.names.is_empty() {
        return make_missing(shared, right.parent, left.names);
    }

    let left_files = sorted_vec::SortedVec::new(left.names);
    let right_files = sorted_vec::SortedVec::new(right.names);
    let names = left_files.merge(right_files);

    names
        .to_iter()
        .map(|name| files_internal(&shared, &left.parent, &right.parent, name))
        .collect()
}
...
fn files_internal(
    ...
    // match files(&left.join(name), &right.join(name))? {
    // Status::Equal => todo!(),
    // Status::Partial => todo!(),
    // Status::MissingLeft => todo!(),
    // Status::MissingRight => todo!(),
    // }
}
...
fn make_missing(
    ...
    let path = path::strip_shared(&prefix, &path)?;

    Ok(names
        .into_iter()
        .map(|name| path::PathStatus {
            path: path.join(name),
            status: path::Status::Missing,
        })
        .collect())
}
